...
(function($) {
	...
	$(window).on('load', function(){
		...
		$('#cs-show-slider-settings').click(function() {
			...
			$('.cs-admin .cs-slider .cs-tabs .active').removeClass('active');
			$(this).parent().addClass('active');
		});
		$('#cs-show-slides').click(function() {
			$('#cs-slides').fadeIn();
			$('#cs-slider-settings').hide();
			//$('.cs-admin .cs-slider .cs-tabs > ul > li').removeClass('active');
			$('.cs-admin .cs-slider .cs-tabs .active').removeClass('active');
			$(this).parent().addClass('active');
		});
		...
		$('.cs-slider').find('#cs-slider-name').keyup(function() {
			var alias = crellyslider_getAlias();
			var shortcode = '';
			shortcode += '<{themebuilderslider_alias="';
			shortcode += alias;
			shortcode += '"}>';
			if(alias != '') {
			...
			update: function(event, ui) {
				...
				$('.cs-admin #cs-slides .cs-slide-tabs > ul > li').each(function() {
					...
					if(!temp.find('a').hasClass('cs-add-new')) {
						temp.find('a').html('<span class="cs-slide-name-text">slide<span class="cs-slide-index">' + (temp.index() + 1) + '</span></span>');
					}
				});
			}
		});
		...
		$(document).on("click", ".cs-admin #cs-slides .cs-slide-tabs > ul > li > a", function() {
      ...
      // Do only if is not click add new
		...
		function crellyslider_addSlide() {
			...
			add_btn.parent().before('<li class="ui-state-default"><a><span class="cs-slide-name-text">slide <span class="cs-slide-index">' + (slides_number + 1) + '</span></span></a><span title="duplicate_slide" class="cs-duplicate"></span><span title="remove_slide" class="cs-close"></span></li>');
      ...
      $('.cs-admin #cs-slider-settings .cs-slider-settings-list #cs-slider-startFromSlide').append('<option value="' + (slides_number - 1) + '">slide' + slides_number + '</option>');
		}
		...
		// Delete
		$(document).on("click", ".cs-admin #cs-slides .cs-slide-tabs > ul > li .cs-close", function() {
			if($('.cs-admin #cs-slides .cs-slide-tabs > ul > li').length <= 2) {
				alert('slide_delete_just_one');
				return;
			}

			var confirm = window.confirm('slide_delete_confirm');
			if(!confirm) {
				return;
			}
    ...
    // Duplicate
	$(document).on("click", ".cs-admin #cs-slides .cs-slide-tabs > ul > li .cs-duplicate", function() {
       var slide_index = $(this).parent().index();
      ...
      // Insert the link at the end of the list
      $(this).parent().parent().find('.cs-add-new').parent().before('<li class="ui-state-default"><a><span class="cs-slide-name-text">slide <span class="cs-slide-index">' + (slides_number + 1) + '</span></span></a><span title="duplicate_slide" class="cs-duplicate"></span><span title="remove_slide" class="cs-close"></span></li>');
      $('.cs-admin #cs-slides .cs-slide-tab').tabs('refresh');
	  ...
	  // Active class
			$('.cs-admin #cs-slides .cs-slide-tabs > ul > li').removeClass('active');
			$('.cs-admin #cs-slides .cs-slide-tabs > ul > li').eq(slides_number - 1).addClass('active');
		...
		//crellyslider_slidesColorPicker();
		crellyslider_slidesColorPicker1();
		function crellyslider_slidesColorPicker1() {
		$('.cs-admin #cs-slides .cs-slides-list .cs-slide .cs-slide-settings-list .cs_slide_background_type_color_picker_input').ColorPicker({
			...
			onChange: function (hsb, hex, rgb) {
				...
				if($(".cs-admin #cs-slides .cs-slides-list .cs-slide:visible").find('input[name="cs-slide-background_type_color"]:checked').val() != '1') {
					$(".cs-admin #cs-slides .cs-slides-list .cs-slide:visible").find('input[name="cs-slide-background_type_color"][value="1"]:radio').prop('checked', true);
				}
				$(".cs-admin #cs-slides .cs-slides-list .cs-slide:visible .cs-slide-settings-list .cs_slide_background_type_color_picker_input").css("backgroundColor", "#" + hex);
				$(".cs-admin #cs-slides .cs-slides-list .cs-slide:visible .cs-slide-settings-list .cs_slide_background_type_color_picker_input").val("#" + hex);

					var area = $(".cs-admin #cs-slides .cs-slides-list .cs-slide:visible").find('.cs-elements .cs-slide-editing-area');
					area.css('background-color', '#' + hex);
			}
			});
		}		
		
				function crellyslider_slidesColorPicker() {
			$('.cs-admin #cs-slides .cs-slides-list .cs-slide:visible .cs-slide-settings-list .cs_slide_background_type_color_picker_input').ColorPicker({
		...
		$('.cs-admin #cs-slides').on('change', '.cs-slides-list .cs-slide-settings-list input[name="cs-slide-background_type_color"]:radio', function() {
			...
			if(btn_val == '0') {
				...
				var color_picker_value = btn.closest('.cs-content').find('.cs_slide_background_type_color_picker_input').css('background-color');
		...
		function crellyslider_addSlideImageBackground(slide_parent) {
			...
			var image_src =slide_parent.find('input[name="background_type_image"]').val();
			// Set background
      ...
      settings = {
        ...
        setup : function(editor) {
          editor.on("KeyUp ExecCommand ContextMenu Change", function (e) {
            ...
            textarea.parent().prepend('<div class="cs-switch-editor"><div onclick="tinyMCE.execCommand(\'mceToggleEditor\', false, \'' + editor.id + '\')" class="cs-button cs-is-default cs-tinymce">switch_editor</div></div>');
          });
  			}
      };
    ...
    // Add text click
		...
		function crellyslider_addTextElement(slide_parent) {
			...
			area.append('<div class="cs-element cs-text-element" style="z-index: 1;">text_element_default_html</div>');
    ...
    crellyslider_changeText = function crellyslider_changeText(element) {
      var index = element.closest('.cs-element-settings').index();
		...
		$('.cs-admin').on('click', '.cs-elements .cs-elements-list .cs-image-element-settings .cs-image-element-upload-button', function() {
			...
			//var image_src =slide_parent.find('input[name="element_type_image"]').val();
		...
		function crellyslider_addImageElement(slide_parent) {
			...
			// Add the image into the editing area.
			  area.append('<img class="cs-element cs-image-element" src="http://localhost/themes/themebuilder/options/img/ico-cross.png" style="z-index: 1;" />');
		...
		function crellySliderUploadImageElement(slide_parent) {
			...
			// Set attributes. If is a link, do the right thing
			  ...
			  var dd1 = slide_parent.find('cs-element-settings .cs-image-element-settings.active');
			  //var dd = dd1.find('.mfn-opts-screenshot');
			  var dd = dd1.find('.mfn-opts-screenshot');
			//var image_src = dd.find('.mfn-opts-screenshot').val();
			//('tag').attr('src');
			//var image_src = dd.attr('src');
			...
			if(image_src != '' && image_src != 'http://localhost/themes/themebuilder/options/img/ico-cross.png') {
				if(! image.is('a')) {
          ...
          element.addClass(html_class);
			...
			$('.cs-slide-live-preview-area a').click(function(event) {
				if($(this).prop('target') != '_blank') {
					var confirm = window.confirm('exit_without_saving');
					if(! confirm) {
		...
		$('.cs-admin .cs-home').on('click', '.cs-sliders-list .cs-delete-slider', function() {
			var confirm = window.confirm('slider_delete_confirm');
			if(!confirm) {
				return;
			}
				...
				success: function(response) {
					...
					if(response !== false) {
						...
						if($('.cs-admin .cs-slider').hasClass('cs-add-slider')) {
							window.location.href = '?fct=themebuilder1&op=slider&action=edit&id=' + response;
							return;
						}else{
			...
			slides.each(function() {
				...
				elements.each(function() {
					...
					var options = {
						...
						//image_src : element.hasClass('cs-image-element-settings') ? element.find('.cs-image-element-upload-button').val() : '',
						image_alt : element.hasClass('cs-image-element-settings') ? element.find('.cs-image-element-upload-button').val() : '',
						image_src : element.hasClass('cs-image-element-settings') ? element.find('input[name="element_type_image"]').val()  : '',
			...
			jQuery.ajax({
				...
				success: function(response) {
					...
					if(response['response'] !== false) {
						...
						cloned_slider.find('.cs-slider-shortcode').html('<{themebuilderslider_alias="' + response['cloned_slider_alias'] + '"}>');
          ...
          //console.log(response);
					if(response['response'] !== false) {
						...
						if(content.length > 0) {
							...
							imported_slider.find('.cs-slider-shortcode').html('<{themebuilderslider_alias="' + response['imported_slider_alias'] + '"}>');
