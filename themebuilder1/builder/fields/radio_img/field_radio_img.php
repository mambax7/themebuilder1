...
class OLIVEE_Options_radio_img
{
    ...
    function render($meta = false)
    {
        ...
        echo "<script type='text/javascript' src='admin/themebuilder1/builder/fields/radio_img/field_radio_img.js' ></script>";
        echo '<fieldset>';
        foreach ($this->field['options'] as $k => $v) {
            echo '<div class="olivee-radio-item">';
            //$selected = (checked($this->value, $k, false) != '')?' olivee-radio-img-selected':'';
            $selected = ($this->value == $v['title'] ? "olivee-radio-img-selected" : "");
            $checked  = ($this->value == $v['title'] ? "checked='checked'" : "");
            //echo $this->value;
            //var_dump ($v['title']);
            ...
            echo '<input type="radio" id="' . $this->field['id'] . '_' . array_search($k, array_keys($this->field['options'])) . '" name="' . $name . '" ' . $class . ' value="' . $k . '" ' . $checked . ' />';
