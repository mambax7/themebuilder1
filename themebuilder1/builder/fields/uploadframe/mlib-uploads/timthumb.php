...
class timthumb
{
    ...
    public function __construct()
    {
        ...
        $this->myHost = preg_replace('/^www\./i', '', (string) $_SERVER['HTTP_HOST']);
        $this->src    = $this->param('src');
        $this->url    = parse_url((string) $this->src);
        $this->src    = preg_replace('/https?:\/\/(?:www\.)?' . $this->myHost . '/i', '', (string) $this->src);

        if (strlen($this->src) <= 3) {
            $this->error("No image specified");
            return false;
        }
        if (BLOCK_EXTERNAL_LEECHERS && array_key_exists('HTTP_REFERER', $_SERVER) && (!preg_match('/^https?:\/\/(?:www\.)?' . $this->myHost . '(?:$|\/)/i', (string) $_SERVER['HTTP_REFERER']))) {
        ...
        if ($this->isURL) {
            if (ALLOW_ALL_EXTERNAL_SITES) {
                ...
                foreach ($ALLOWED_SITES as $site) {
                    if ((strtolower(substr($this->url['host'], -strlen((string) $site) - 1)) === strtolower(".$site")) || (strtolower($this->url['host']) === strtolower((string) $site))) {
        ...
        if ($this->isURL) {
            $arr = explode('&', (string) $_SERVER ['QUERY_STRING']);
            asort($arr);
    ...
    protected function tryBrowserCache()
    {
        ...
        if (!empty($_SERVER['HTTP_IF_MODIFIED_SINCE'])) {
            ...
            $iftime = strtotime((string) $_SERVER['HTTP_IF_MODIFIED_SINCE']);
    ...
    protected function serveErrors()
    {
        ...
        foreach ($this->errors as $err) {
            $html .= '<li>' . htmlentities((string) $err) . '</li>';
        }
        $html .= '</ul>';
        echo '<h1>A TimThumb error has occured</h1>The following error(s) occured:<br />' . $html . '<br />';
        echo '<br />Query String : ' . htmlentities((string) $_SERVER['QUERY_STRING'], ENT_QUOTES);
    ...
    protected function processImageAndWriteToCache($localImage)
    {
        ...
        if (strlen((string) $canvas_color) == 3) { //if is 3-char notation, edit string into 6-char notation
            $canvas_color = str_repeat(substr((string) $canvas_color, 0, 1), 2) . str_repeat(substr((string) $canvas_color, 1, 1), 2) . str_repeat(substr((string) $canvas_color, 2, 1), 2);
        } elseif (strlen((string) $canvas_color) != 6) {
            $canvas_color = DEFAULT_CC; // on error return default canvas color
        }

        $canvas_color_R = hexdec(substr((string) $canvas_color, 0, 2));
        $canvas_color_G = hexdec(substr((string) $canvas_color, 2, 2));
        $canvas_color_B = hexdec(substr((string) $canvas_color, 4, 2));
        ...
        if ($zoom_crop > 0) {
            ...
            // positional cropping!
            if ($align) {
                if (str_contains((string) $align, 't')) {
                    $src_y = 0;
                }
                if (str_contains((string) $align, 'b')) {
                    $src_y = $height - $src_h;
                }
                if (str_contains((string) $align, 'l')) {
                    $src_x = 0;
                }
                if (str_contains((string) $align, 'r')) {
                    $src_x = $width - $src_w;
                }
            }
        ...
        if ($filters != '' && function_exists('imagefilter') && defined('IMG_FILTER_NEGATE')) {
            ...
            $filterList = explode('|', (string) $filters);
            foreach ($filterList as $fl) {
                ...
                if (isset ($imageFilters[$filterSettings[0]])) {
                    ...
                    match ($imageFilters[$filterSettings[0]][1]) {
                        1 => imagefilter($canvas, $imageFilters[$filterSettings[0]][0], $filterSettings[1]),
                        2 => imagefilter($canvas, $imageFilters[$filterSettings[0]][0], $filterSettings[1], $filterSettings[2]),
                        3 => imagefilter($canvas, $imageFilters[$filterSettings[0]][0], $filterSettings[1], $filterSettings[2], $filterSettings[3]),
                        4 => imagefilter($canvas, $imageFilters[$filterSettings[0]][0], $filterSettings[1], $filterSettings[2], $filterSettings[3], $filterSettings[4]),
                        default => imagefilter($canvas, $imageFilters[$filterSettings[0]][0]),
                    };
                }
            }
        }
    ...
    protected function calcDocRoot()
    {
        ...
        if (!isset($docRoot)) {
            ...
            if (isset($_SERVER['SCRIPT_FILENAME'])) {
                $docRoot = str_replace('\\', '/', substr((string) $_SERVER['SCRIPT_FILENAME'], 0, 0 - strlen((string) $_SERVER['PHP_SELF'])));
        ...
        if (!isset($docRoot)) {
            ...
            if (isset($_SERVER['PATH_TRANSLATED'])) {
                $docRoot = str_replace('\\', '/', substr(str_replace('\\\\', '\\', (string) $_SERVER['PATH_TRANSLATED']), 0, 0 - strlen((string) $_SERVER['PHP_SELF'])));
                $this->debug(3, "Generated docRoot using PATH_TRANSLATED and PHP_SELF as: $docRoot");
            }
        }
        if ($docRoot && $_SERVER['DOCUMENT_ROOT'] != '/') {
            $docRoot = preg_replace('/\/$/', '', (string) $docRoot);
        }
    ...
    {
        $src = ltrim((string) $src, '/'); //strip off the leading '/'
        if (!$this->docRoot) {
        ...
        if (file_exists($this->docRoot . '/' . $src)) {
            ...
            if (stripos((string) $real, (string) $this->docRoot) === 0) {
                return $real;
            } else {
        ...
        if ($absolute && file_exists($absolute)) { //realpath does file_exists check, so can probably skip the exists check here
            ...
            if (stripos((string) $absolute, (string) $this->docRoot) === 0) {
                return $absolute;
            } else {
                $this->debug(1, "Security block: The file specified occurs outside the document root.");
                //and continue search
        ...
        if (strstr((string) $_SERVER['SCRIPT_FILENAME'], ':')) {
            $sub_directories = explode('\\', str_replace($this->docRoot, '', (string) $_SERVER['SCRIPT_FILENAME']));
        } else {
            $sub_directories = explode('/', str_replace($this->docRoot, '', (string) $_SERVER['SCRIPT_FILENAME']));
        }
        ...
        foreach ($sub_directories as $sub) {
            ...
            if (file_exists($base . $src)) {
                ...
                if (stripos((string) $real, (string) $this->realpath($this->docRoot)) === 0) {
                    return $real;
                } else {
                    $this->debug(1, "Security block: The file specified occurs outside the document root.");
                    //And continue search
    ...
    protected function realpath($path)
    {
        ...
        $remove_relatives = '/\w+\/\.\.\//';
        while (preg_match($remove_relatives, (string) $path)) {
            $path = preg_replace($remove_relatives, '', (string) $path);
        }
        ...
        return preg_match('#^\.\./|/\.\./#', (string) $path) ? realpath($path) : $path;
    }
    ...
    protected function serveWebshot()
    {
        ...
        $url       = $this->src;
        if (!preg_match('/^https?:\/\/[a-zA-Z0-9\.\-]+/i', (string) $url)) {
            return $this->error("Invalid URL supplied.");
        }
        $url = preg_replace('/[^A-Za-z0-9\-\.\_:\/\?\&\+\;\=]+/', '', (string) $url); //RFC 3986 plus ()$ chars to prevent exploit below. Plus the following are also removed: @*!~#[]',
    ...
    {
        if (!preg_match('/^https?:\/\/[a-zA-Z0-9\-\.]+/i', (string) $this->src)) {
        ...
        if (!preg_match("/^image\/(?:jpg|jpeg|gif|png)$/i", (string) $mimeType)) {
    ...
    {
        fwrite(self::$curlFH, (string) $d);
        self::$curlDataWritten += strlen((string) $d);
        if (self::$curlDataWritten > MAX_FILE_SIZE) {
            return 0;
        } else {
            return strlen((string) $d);
        }
    }
    ...
    protected function serveCacheFile()
    {
        ...
        fseek($fp, strlen((string) $this->filePrependSecurityBlock), SEEK_SET);
        $imgType = fread($fp, 3);
        fseek($fp, 3, SEEK_CUR);
        if (ftell($fp) != strlen((string) $this->filePrependSecurityBlock) + 6) {
            @unlink($this->cachefile);
            return $this->error("The cached image file seems to be corrupt.");
        }
        $imageDataSize = filesize($this->cachefile) - (strlen((string) $this->filePrependSecurityBlock) + 6);
        ...
        if ($content != false) {
            $content = substr($content, strlen((string) $this->filePrependSecurityBlock) + 6);
            echo $content;
    ...
    {
        if (!preg_match('/^image\//i', (string) $mimeType)) {
            $mimeType = 'image/' . $mimeType;
        }
        if (strtolower((string) $mimeType) == 'image/jpg') {
            $mimeType = 'image/jpeg';
        }
    ...
    protected function getIP()
    {
        ...
        if (preg_match('/^(?:192\.168|172\.16|10\.|127\.)/', (string) $rem)) {
            if ($ff) {
                return $ff;
            }
            if ($ci) {
    ...
    protected function getURL($url, $tempfile)
    {
        ...
        $url                = preg_replace('/ /', '%20', (string) $url);
