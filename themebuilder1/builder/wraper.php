...
if( ! function_exists( 'mfn_builder_section' ) )
{
	 ...
	 */
	function mfn_builder_section( $section = false, $section_id = false, &$wrap_id = false, $item_std ) {
				...
				echo '<div class="mfn-element-header mfn-wrap-header">';
					...
					if( $wrap && key_exists('items', $wrap) && is_array($wrap['items']) ){
						foreach( $wrap['items'] as $item ){
							mfn_builder_item( $item['type'], $item, $wrap_id, $item_std);
						}
					}
			...
			echo '<div class="mfn-element-content">';
				...
				echo '<div class="mfn-item-content">';
					...
					if( $item && key_exists( 'fields', $item ) && key_exists( 'content', $item['fields'] ) ){
						...
						if(substr($item['fields']['content'] , 0, 3) == '<{$'){
							$item_excerpt = substr($item['fields']['content'], 3, -2);
							}else{
							$item_excerpt = $item['fields']['content'];
							}
						$item_excerpt = strip_tags( $item_excerpt );
		...
		if ($form == 'pagebuilder' || $form == 'layoutbuilder'){
			...
			if ($pageid == false) {
				if ($form == 'layoutbuilder') {
					$titlee .= '<div class="">Titre du layout: <input class="text-input" size="35" name="mfn-item-titre" type="text" placeholder="Your layout titre" data-msg-required="This field is required." required></div>';				
				}else{
					$titlee .= '<div class="">Titre de la page: <input class="text-input" size="35" name="mfn-item-titre" type="text" placeholder="Your page titre" data-msg-required="This field is required." required></div>';
					...
					$titlee .= '<br><label><input name="selection" type="radio" value="3"> titre de la page SEO</label><code>'.XOOPS_URL.'/sample-post/</code>';
				}
			}else{
				...
				if ($form == 'layoutbuilder') {
					$titlee .= '<div class="">Titre du layout: <input class="text-input" size="35" name="mfn-item-titre" value="'.$head_arrl['conf_title'].'" type="text" placeholder="Your layout titre" data-msg-required="This field is required." required></div>';
				}else{
					$titlee .= '<div class="">Titre de la page: <input class="text-input" size="35" name="mfn-item-titre" value="'.$head_arrl['conf_title'].'" type="text" placeholder="Your page titre" data-msg-required="This field is required." required></div>';
				...
				<div id="mfn-items" class="clearfix">
					<?php
						...
						foreach( $items as $item ){
							mfn_builder_item( $item['type'], $item = false, $wrap_id = false, $item_std );
							echo "\n";
						}
					?>
				</div>
					...
					<div class="migrate-wrapper templates-wrapper hide">
						...
						<select id="mfn-items-import-template">
							...
							<?php
								...
								$result3111 = $xoopsDB -> query($sql3111); 
								$templates = array();
								while (list($conf_id, $conf_title) = $xoopsDB -> fetchRow($result3111)) {
	...
	function mfn_builder_insert($form, $pageid, $new, $olives) {
		...
		if ($form == 'pagebuilder' || $form == 'layoutbuilder'){
			if ($pageid == false) {
				...
				$vowels = array(" ", "  ");
				$_POST['mfn-item-titre'] = str_replace($vowels, "-", $_POST['mfn-item-titre']);
				//var_dump($pageid);
				...
				$vowels = array(" ", "  ");
				if ($form == 'layoutbuilder') {$_POST['selection'] = '';}
				$_POST['mfn-item-titre'] = str_replace($vowels, "-", $_POST['mfn-item-titre']);
				$sqlr = "UPDATE " . $xoopsDB -> prefix( 'config_theme' ) . " SET conf_modid = '".$_POST['selection']."', conf_title = '".$_POST['mfn-item-titre']."', conf_value = '".$new."', conf_desc = '".addslashes ($olives)."' WHERE conf_id = '".$pageid."'";
