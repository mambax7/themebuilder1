...
$exporterid = $_POST['exporterid'] ?? $_GET['exporterid'];
$menuid     = $_GET['menuid'];
if ($exporterid == 'menu') {
    echo 'export menu
...
array with all level of menu ';
    ...
    while ([$id, $label, $catmenu, $link, $parent, $sort, $icons, $class] = $xoopsDB->fetchRow($result)) {
        $video_array1['id']     = $id;
    ...
    readfile($file);
    echo '</textarea></p>
												<p><a href="admin/themebuilder1/down.php?fct=themebuilder&op=exporter&exporterid=menu&menuid=' . $menuid . '&action=download" class="button-secondary">Download as file</a></p>
...
if ($exporterid == 'option') {
    ...
    readfile($file);
    echo '</textarea></p>
												<p><a href="admin/themebuilder1/down.php?fct=themebuilder&op=exporter&exporterid=option&action=download" class="button-secondary">Download as file</a></p>
							';
}

if ($exporterid == 'template') {
    $templateid = $_POST['templateid'] ?? $_GET['templateid'];
    ...
    readfile($file);
    echo '</textarea></p>
												<p><a href="admin/themebuilder1/down.php?fct=themebuilder&op=exporter&exporterid=template&templateid=' . $templateid . '&action=download" class="button-secondary">Download as file</a></p>
												';
}
if ($exporterid == 'page') {
    //echo 'page';
    ...
    readfile($file);
    echo '</textarea></p>
												<p><a href="admin/themebuilder1/down.php?fct=themebuilder&op=exporter&exporterid=page&pageid=' . $pageid . '&action=download" class="button-secondary">Download as file</a></p>
												';
}
?>
