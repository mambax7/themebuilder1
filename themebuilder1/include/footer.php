...
global $xoopsDB;
	...
	echo '</table>';
		...
		if ($ok == 1){
			...
			xoops_confirm(array('footerid' => $_GET['footerid'], 'ok' => 1), 'admin.php?fct=themebuilder&op=delfooter', _AM_ARABESK125dotNET_OOOOOOOOOOOO_AREUSURE);
		}
		...
		if ($ok == 1 && $_POST['confirm_submit'] == 'clone'){
			...
			$resultexists = $xoopsDB -> fetchArray( $resultexist );
				...
				}else{$message = _AM_FOOTER_ID_PROB_SECURITY;}
			...
			xoops_confirmk(array('footerid' => $_GET['footerid'], 'ok' => 1), 'admin.php?fct=themebuilder&op=clonefooter', '<div class="">Titre du footerbar: <input class="text-input" size="130" name="mfn-item-titre" type="text" placeholder="Your footerbar titre" data-msg-required="This field is required." required=""></div>');
		}
