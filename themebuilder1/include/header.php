...
global $xoopsDB;
	...
	echo '</table>';
		...
		if ($ok == 1){
			...
			xoops_confirm(array('headerid' => $_GET['headerid'], 'ok' => 1), 'admin.php?fct=themebuilder&op=delheader', _AM_ARABESK125dotNET_OOOOOOOOOOOO_AREUSURE);
		}
		...
		if ($ok == 1 && $_POST['confirm_submit'] == 'clone'){
			...
			$resultexists = $xoopsDB -> fetchArray( $resultexist );
				...
				}else{$message = _AM_HEADER_ID_PROB_SECURITY;}
			...
			xoops_confirmk(array('headerid' => $_GET['headerid'], 'ok' => 1), 'admin.php?fct=themebuilder&op=cloneheader', '<div class="">Titre du header: <input class="text-input" size="130" name="mfn-item-titre" type="text" placeholder="Your headerbar titre" data-msg-required="This field is required." required=""></div>');
		}
