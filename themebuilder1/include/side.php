...
switch ($op) {
    case 'delside':
        ...
        if ($ok == 1) {
            ...
            xoops_confirm(['sideid' => $_GET['sideid'], 'ok' => 1], 'admin.php?fct=themebuilder1&op=side&action=delside', \_AM_ARABESK125DOTNET_OOOOOOOOOOOO_AREUSURE);
        }
    ...
    case 'cloneside':
        ...
        if ($ok == 1 && $_POST['confirm_submit'] == 'clone') {
            ...
            if ($resultexists['conf_title'] != $_POST['mfn-item-titre']) {
                ...
                $message = _AM_SIDE_ID_PROB_SECURITY;
            }
            ...
            xoops_confirmk(['sideid' => $_GET['sideid'], 'ok' => 1], 'admin.php?fct=themebuilder1&op=side&action=cloneside', '<div class="">Sidebar Title: <input class="text-input" size="130" name="mfn-item-titre" type="text" placeholder="Your sidebar titre" data-msg-required="This field is required." required=""></div>');
        }
    ...
    case 'modside':
        ...
        $sideid = $_POST['sideid'] ?? $_GET['sideid'];
        echo 'mod' . $sideid;
        sidebuilder($sideid);
