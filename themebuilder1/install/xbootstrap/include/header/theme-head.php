		...
		if( $htmlB ){
			...
			if( $htmlP ){
				...
				if( $background_attr[3] ){
					...
					if( in_array( $xoTheme->template->_tpl_vars[ 'size-page-bg' ], array( 'cover', 'contain' ) ) ){
		...
		if( $headerB ){
			...
			if( $headerP == "fixed" ){
				...
				if( $background_attr[3] ){
					...
					if( in_array( $xoTheme->template->_tpl_vars[ 'size-subheader-bg' ], array( 'cover', 'contain' ) ) ){
		...
		if( $topbarB ){
			...
			if( $topbarP ){
				...
				if( $background_attr[3] ){
					...
					if( in_array( $xoTheme->template->_tpl_vars[ 'topbar-bg-img-size' ], array( 'cover', 'contain' ) ) ){
		...
		if( $subheaderB ){
			...
			if( $subheaderP ){
				...
				if( $background_attr[3] ){
					...
					if( in_array( $xoTheme->template->_tpl_vars[ 'subheader-size' ], array( 'cover', 'contain' ) ) ){
		...
		if( $footerB ){
			...
			if( $footerP ){
				...
				if( $background_attr[3] ){
					...
					if( in_array( $xoTheme->template->_tpl_vars[ 'footer-bg-img-size' ], array( 'cover', 'contain' ) ) ){
...
if( ! function_exists( 'mfn_header_style' ) )
{
	function mfn_header_style( $firstPartOnly = false ){
		...
		} else*/if( $xoTheme->template->_tpl_vars['header-style'] ){
...
if( ! function_exists( 'body_class' ) )
{
	function body_class(  )
	{
		...
		// Nice Scroll ----------------------------------------
		if( $xoTheme->template->_tpl_vars[ 'nice-scroll' ] == '1' ) $classes .= ' nice-scroll-on';


		// Image Frame | Style --------------------------------
		/*if( $_GET && key_exists( 'mfn-if', $_GET ) ){
			$classes .= 'if-'. esc_html( $_GET['mfn-if'] ); // demo
		} else*/if( $xoTheme->template->_tpl_vars['image-frame-style'] ){
		...
		// Image Frame | Caption -------------------------------
		if( $xoTheme->template->_tpl_vars['image-frame-caption'] ) $classes .= ' if-caption-on';
		...
		// Table Hover ----------------------------------------
		if( $xoTheme->template->_tpl_vars['table-hover'] ) $classes .= ' table-'. $xoTheme->template->_tpl_vars['table-hover'];


		// Contact Form 7 | Form Error ------------------------
		if( $xoTheme->template->_tpl_vars[ 'cf7-error' ] ) $classes .= ' cf7p-'. $xoTheme->template->_tpl_vars[ 'cf7-error' ];
		...
		$header_options = $xoTheme->template->_tpl_vars[ 'header-fw' ] ? $xoTheme->template->_tpl_vars[ 'header-fw' ] : false;
		...
		} else*/if( isset( $header_options['full-width'] )  ){
			$classes .= ' header-fw';
		}
		...
		} else*/if( $xoTheme->template->_tpl_vars['minimalist-header'] == 'no' ) {
			$classes .= ' minimalist-header-no';
		...
		} else*/if( $xoTheme->template->_tpl_vars['sticky-header'] ){
			$classes .= ' sticky-header';
		}
		...
		} else*/if( ! in_array( $skin, array('custom','one') ) ){
		...
		} else*/if( $xoTheme->template->_tpl_vars['menu-style'] ) {
		...
		} else*/if( $xoTheme->template->_tpl_vars['footer-style'] ) {
...
if( ! function_exists( 'mfn_slider' ) )
{
	...
	$rest = substr($xoTheme->template->_tpl_vars[ 'slider-shortcode' ], 3, -2);
	$id = substr($rest, -2);
...
//var_dump($slides);
		...
		$output .= '<link rel="stylesheet" href="'. XOOPS_URL . '/modules/system/admin/themebuilder/slider/css/crellyslider.css" type="text/css" media="all">';
		$output .= '<script type="text/javascript" src="'. XOOPS_URL . '/modules/system/admin/themebuilder/slider/js/jquery.crellyslider.js"></script>';
		...
		foreach($slides as $slide) {
			...
			$customcsss = $slide['custom_css'] ? stripslashes($slide['custom_css']) . "\n" : '';
			$output .= '<li' .  "\n" .
			'style="' . "\n" .
