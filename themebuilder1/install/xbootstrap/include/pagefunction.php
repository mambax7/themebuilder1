...
if (!function_exists('sc_promo_box')) {
    function sc_promo_box($attr, $content = null)
    {
        ...
        if ($attr['btn_link']) {
            $output .= '<a href="' . $attr['btn_link'] . '" class="button button_left button_theme button_js" ' . $target . '><span class="button_icon"><i class="icon-layout"></i></span><span class="button_label">' . $attr['btn_text'] . '</span></a>';
        }
        $output .= '</div>';
...
if (!function_exists('sc_blog')) {
    function sc_blog($attr, $content = null)
    {
        ...
        if ($pagination) {
            $output .= mfn_pagination($query_blog);
        }
    ...
    function sc_blog_slider($attr, $content = null)
    {
        ...
        while ($query_blog->have_posts()) {
            ...
            if (get_post_format() == 'quote') {
                ...
                $output .= '<div class="image_wrapper">';
                $output .= get_the_post_thumbnail(get_the_ID(), 'blog', ['class' => 'scale-with-grid']);
                $output .= '</div>';
                $output .= '</div>';
            }
...
if (!function_exists('sc_shop_slider')) {
    function sc_shop_slider($attr, $content = null)
    {
        ...
        while ($query_shop->have_posts()) {
            ...
            $output .= '<div class="mask"></div>';
            $output .= get_the_post_thumbnail(null, 'shop_catalog', ['class' => 'scale-with-grid']);
            $output .= '</a>';
    ...
    function sc_clients($attr, $content = null)
    {
        ...
        if ($clients_query->have_posts()) {
            ...
            while ($clients_query->have_posts()) {
                ...
                if ($link) {
                    $output .= '<a target="_blank" href="' . $link . '" title="' . the_title(false, false, false) . '">';
                }
                $output .= get_the_post_thumbnail(null, 'clients-slider', ['class' => 'scale-with-grid']);
                if ($link) {
                    $output .= '</a>';
                }
                $output .= '</div>';
...
if (!function_exists('fancy_heading')) {
    function sc_fancy_heading($attr, $content = null)
    {
        ...
        if ($attr['style'] == 'arrows') {
            $attr['title'] = '<i class="icon-right-dir"></i>' . $attr['title'] . '<i class="icon-left-dir"></i>';
        }
        if ($attr['title']) {
            if ($h1) {
...
if (!function_exists('sc_portfolio')) {
    function sc_portfolio($attr, $content = null)
    {
        ...
        if ($pagination) {
            $output .= mfn_pagination($query_portfolio);
        }
    ...
    function sc_offer($attr = false, $content = null)
    {
        ...
        if ($offer_query->have_posts()) {
            ...
            while ($offer_query->have_posts()) {
                ...
                $output .= '<div class="image_wrapper">';
                $output .= get_the_post_thumbnail(get_the_ID(), 'full', ['class' => 'scale-with-grid']);
                $output .= '</div>';
                ...
                if ($link) {
                    $output .= '<a href="' . $link . '" class="button button_js" ' . $target . '><span class="button_icon"><i class="icon-layout"></i></span><span class="button_label">' . get_post_meta(get_the_ID(), 'mfn-post-link_title', true) . '</span></a>';
                }
                $output .= '</div>';
        ...
        if ($query_tm->have_posts()) {
            ...
            while ($query_tm->have_posts()) {
                ...
                if (has_post_thumbnail()) {
                    $output .= get_the_post_thumbnail(null, 'testimonials', ['class' => 'scale-with-grid']);
                } else {
...
function mfn_builder_item($mfn_items)
{
    $classes = [
        '1/4' => 'col-sm-3 col-md-3',
        '1/3' => 'col-sm-4 col-md-4',
        '1/2' => 'col-sm-6 col-md-6',
        '2/3' => 'col-sm-9 col-md-8',
        '3/4' => 'col-sm-8 col-md-9',
        '1/1' => 'col-sm-12 col-md-12',
    ];
    if (is_array($mfn_items)) {
        ...
        foreach ($mfn_items as $section) {
            ...
            $olive .= '<div class="section' . $section_class . '" ' . $section_id . ' style="' . $section_style . '" ' . $parallax . '>'; // 100%
            $olive .= '<div class="section_wrapper clearfix">'; // WIDTH + margin: 0 auto

            // Items ------------------------
            $olive .= '<div class="items_group clearfix">'; // 100% || WIDTH (sidebar)
            if (is_array($section['items'])) {
                foreach ($section['items'] as $item) {
                    if (function_exists('mfn_print_' . $item['type'])) {
                        ...
                        $olive .= '<div class="column ' . $class . '">';
                        $olive .= call_user_func('mfn_print_' . $item['type'], $item);
                        //$olivek = call_user_func( 'mfn_print_'. $item['type'], $item );
                        $olive .= '</div>';
                        //var_dump($item);
                    } else {
                        $olive .= ' pas de class mfn_print_' . $item['type'];
                    }
                }
            }
            $olive .= '</div>';
