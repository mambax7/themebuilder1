...
if (!function_exists('sc_article_box')) {
    function sc_article_box($attr, $content = null)
    {
        ...
        [$width, $height] = getimagesize('http://localhost' . $image);
...
if (!function_exists('sc_helper')) {
    function sc_helper($attr, $content = null)
    {
        ...
        if (!$link1) {
            $output .= '<div class="item item-1">' . $content1 . '</div>';
        }

        if (!$link2) {
            $output .= '<div class="item item-2">' . $content2 . '</div>';
        }
...
if (!function_exists('sc_before_after')) {
    function sc_before_after($attr, $content = null)
    {
        ...
        [$width, $height] = getimagesize('http://localhost' . $image_before);
        [$width1, $height1] = getimagesize('http://localhost' . $image_after);
...
if (!function_exists('sc_flat_box')) {
    function sc_flat_box($attr, $content = null)
    {
        ...
        [$width, $height] = getimagesize('http://localhost' . $image);
        [$width1, $height1] = getimagesize('http://localhost' . $icon_image);
...
if (!function_exists('sc_zoom_box')) {
    function sc_zoom_box($attr, $content = null)
    {
        ...
        [$width, $height] = getimagesize('http://localhost' . $image);
        [$width1, $height1] = getimagesize('http://localhost' . $content_image);
...
if (!function_exists('sc_trailer_box')) {
    function sc_trailer_box($attr, $content = null)
    {
        ...
        if ($slogan) {
            $output .= '<div class="subtitle">' . $slogan . '</div>';
        }
        if ($title) {
            $output .= '<h2>' . $title . '</h2>';
        }
...
if (!function_exists('sc_promo_box')) {
    function sc_promo_box($attr, $content = null)
    {
        ...
        if ($btn_link) {
            $output .= '<a href="' . $btn_link . '" class="button button_left button_theme button_js" ' . $target . '><span class="button_icon"><i class="icon-layout"></i></span><span class="button_label">' . $btn_text . '</span></a>';
        }
        $output .= '</div>';
...
if (!function_exists('sc_contact_box')) {
    function sc_contact_box($attr, $content = null)
    {
        ...
        if ($telephone) {
            ...
            $output .= '<p><a href="tel:' . str_replace(' ', '', (string) $telephone) . '">' . $telephone . '</a></p>';
            $output .= '</li>';
        }
        if ($telephone_2) {
            $output .= '<li class="phone phone-2">';
            $output .= '<span class="icon"><i class="icon-phone"></i></span>';
            $output .= '<p><a href="tel:' . str_replace(' ', '', (string) $telephone_2) . '">' . $telephone_2 . '</a></p>';
            $output .= '</li>';
        }
        if ($fax) {
            $output .= '<li class="phone fax">';
            $output .= '<span class="icon"><i class="icon-print"></i></span>';
            $output .= '<p><a href="fax:' . str_replace(' ', '', (string) $fax) . '">' . $fax . '</a></p>';
            $output .= '</li>';
        }
        if ($email) {
        ...
        if ($www) {
            if (str_starts_with((string) $www, 'http')) {
                $url = $www;
                $www = str_replace('http://', '', (string) $www);
...
if (!function_exists('sc_call_to_action')) {
    function sc_call_to_action($attr, $content = null)
    {
        ...
        // FIX | prettyphoto
        if (str_contains((string) $class, 'prettyphoto')) {
            $class = str_replace('prettyphoto', '', (string) $class);
            $rel   = 'rel="prettyphoto"';
        } else {
            $rel = false;
        }
        ...
        if ($button_title) {
            // Button
            if ($link) {
                $output .= '<a href="' . $link . '" class="button button_js ' . $class . '" ' . $rel . ' ' . $target . '>';
            }
            if ($icon) {
            ...
            // Big Icon Link
            if ($link) {
                $output .= '<a href="' . $link . '" class="' . $class . '" ' . $rel . ' ' . $target . '>';
            }
...
if (!function_exists('sc_chart')) {
    function sc_chart($attr, $content = null)
    {
        ...
        // line width
        if ($line_width) {
            $line_width = 'data-line-width="' . intval($line_width) . '"';
        }
...
if (!function_exists('sc_counter')) {
    function sc_counter($attr, $content = null)
    {
        ...
        if ($number) {
            $output .= '<div class="number-wrapper">';
            if ($prefix) {
                $output .= '<span class="label prefix">' . $prefix . '</span>';
            }
            $output .= '<span class="number" data-to="' . $number . '">' . $number . '</span>';
            if ($label) {
                $output .= '<span class="label postfix">' . $label . '</span>';
            }
            $output .= '</div>';
        }
        if ($title) {
...
if (!function_exists('sc_image')) {
    function sc_image($attr, $content = null)
    {
        ...
        if ($margin || $margin_bottom) {
            $margin_tmp = '';
            if ($margin) {
                $margin_tmp .= 'margin-top:' . intval($margin) . 'px;';
            }
            if ($margin_bottom) {
                $margin_tmp .= 'margin-bottom:' . intval($margin_bottom) . 'px;';
            }
        ...
        if ($link || $link_image) {
            ...
            if ($link_image) {
                $output .= '<a href="' . $link_image . '" class="zoom" rel="prettyphoto"><i class="icon-search"></i></a>';
            }
            if ($link) {
                $output .= '<a href="' . $link . '" class="link" ' . $target . '><i class="icon-link"></i></a>';
            }
            $output .= '</div>';
...
if (!function_exists('sc_hover_box')) {
    function sc_hover_box($attr, $content = null)
    {
        ...
        [$width, $height] = getimagesize('http://localhost' . $image);
        [$width1, $height1] = getimagesize('http://localhost' . $image_hover);
...
if (!function_exists('sc_quick_fact')) {
    function sc_quick_fact($attr, $content = null)
    {
        ...
        if ($heading) {
            $output .= '<h4 class="title">' . $heading . '</h4>';
        }
        if ($number) {
            $output .= '<div class="number-wrapper">';
            if ($prefix) {
...
if (!function_exists('sc_button')) {
    function sc_button($attr, $content = null)
    {
        ...
        if ($align) {
            $output .= '<div class="button_align align_' . $align . '">';
        }

        $output .= '<a class="button ' . $class . ' button_js" href="' . $link . '" ' . $attributes . '>';
        if ($icon) {
            $output .= '<span class="button_icon"><i class="' . $icon . '" ' . $style_icon . '></i></span>';
        }
        if ($title) {
...
if (!function_exists('sc_fancy_heading')) {
    function sc_fancy_heading($attr, $content = null)
    {
        ...
        if ($style == 'icon' && $icon) {
            $output .= '<span class="icon_top"><i class="' . $icon . '"></i></span>';
        }

        if ($style == 'line' && $slogan) {
            $output .= '<span class="slogan">' . $slogan . '</span>';
        }

        if ($style == 'arrows') {
            $title = '<i class="icon-right-dir"></i>' . $title . '<i class="icon-left-dir"></i>';
        }
...
if (!function_exists('sc_icon_box')) {
    function sc_icon_box($attr, $content = null)
    {
        ...
        if ($link) {
            $output .= '<a class="' . $class . '" href="' . $link . '" ' . $target . ' ' . $rel . '>';
        }
...
if (!function_exists('sc_our_team')) {
    function sc_our_team($attr, $content = null)
    {
        ...
        if ($phone) {
            $output .= '<p class="phone"><i class="icon-phone"></i> <a href="tel:' . $phone . '">' . $phone . '</a></p>';
        }
        $output .= '<hr class="hr_color" />';
        ...
        if ($email || $phone || $facebook || $twitter || $linkedin) {
            $output .= '<div class="links">';
            if ($email) {
                $output .= '<a href="mailto:' . $email . '" class="icon_bar icon_bar_small"><span class="t"><i class="icon-mail"></i></span><span class="b"><i class="icon-mail"></i></span></a>';
            }
            if ($facebook) {
                $output .= '<a target="_blank" href="' . $facebook . '" class="icon_bar icon_bar_small"><span class="t"><i class="icon-facebook"></i></span><span class="b"><i class="icon-facebook"></i></span></a>';
            }
            if ($twitter) {
                $output .= '<a target="_blank" href="' . $twitter . '" class="icon_bar icon_bar_small"><span class="t"><i class="icon-twitter"></i></span><span class="b"><i class="icon-twitter"></i></span></a>';
            }
            if ($linkedin) {
                $output .= '<a target="_blank" href="' . $linkedin . '" class="icon_bar icon_bar_small"><span class="t"><i class="icon-linkedin"></i></span><span class="b"><i class="icon-linkedin"></i></span></a>';
            }
            if ($vcard) {
                $output .= '<a href="' . $vcard . '" class="icon_bar icon_bar_small"><span class="t"><i class="icon-vcard"></i></span><span class="b"><i class="icon-vcard"></i></span></a>';
            }
            $output .= '</div>';
        }
    ...
    function sc_Blockcolumn($attr, $content = null)
    {
        ...
        if ($attr['content'] == 'Center') {
            ...
            <div class="row">';
                ...
                $_from  = $xoTheme->template->_tpl_vars['xoBlocks']['page_topleft'];
                if (!is_array($_from) && !is_object($_from)) {
                    settype($_from, 'array');
                }
                if (count($_from)):
                ...
                $output .= '<aside class="col-sm-' . $toe . ' col-md-' . $toe . '">';
                $_from  = $xoTheme->template->_tpl_vars['xoBlocks']['page_topcenter'];
                if (!is_array($_from) && !is_object($_from)) {
                    settype($_from, 'array');
                }
                if (count($_from)):
                ...
                $output .= '<aside class="col-sm-' . $toe . ' col-md-' . $toe . '">';
                $_from  = $xoTheme->template->_tpl_vars['xoBlocks']['page_topright'];
                if (!is_array($_from) && !is_object($_from)) {
                    settype($_from, 'array');
                }
                if (count($_from)):
...
<!-- Start content module page -->';*/
            ...
            if ($xoTheme->template->_tpl_vars['xoBlocks']['page_bottomcenter'] || $xoTheme->template->_tpl_vars['xoBlocks']['page_bottomright'] || $xoTheme->template->_tpl_vars['xoBlocks']['page_bottomleft']) {
                ...
                if ($xoTheme->template->_tpl_vars['xoBlocks']['page_bottomleft']) {
                    $output .= '<aside class="col-sm-' . $toe . ' col-md-' . $toe . '">';
                    $_from  = $xoTheme->template->_tpl_vars['xoBlocks']['page_bottomleft'];
                    if (!is_array($_from) && !is_object($_from)) {
                        settype($_from, 'array');
                    }
                    if (count($_from)):
                ...
                if ($xoTheme->template->_tpl_vars['xoBlocks']['page_bottomcenter']) {
                    $output .= '<aside class="col-sm-' . $toe . ' col-md-' . $toe . '">';
                    $_from  = $xoTheme->template->_tpl_vars['xoBlocks']['page_bottomcenter'];
                    if (!is_array($_from) && !is_object($_from)) {
                        settype($_from, 'array');
                    }
                    if (count($_from)):
                ...
                if ($xoTheme->template->_tpl_vars['xoBlocks']['page_bottomright']) {
                    $output .= '<aside class="col-sm-' . $toe . ' col-md-' . $toe . '">';
                    $_from  = $xoTheme->template->_tpl_vars['xoBlocks']['page_bottomright'];
                    if (!is_array($_from) && !is_object($_from)) {
                        settype($_from, 'array');
                    }
                    if (count($_from)):
...
if (!function_exists('sc_map_basic')) {
    function sc_map_basic($attr, $content = null)
    {
        ...
        if ($iframe) {
            ...
            $address = str_replace([' ', ','], '+', trim((string) $address));
            $api_key = trim((string) $xoTheme->template->_tpl_vars['google-maps-api-key']);
...
if (!function_exists('sc_map')) {
    function sc_map($attr, $content = null)
    {
        ...
        if (str_contains((string) $controls, 'zoom')) {
            $zoomControl = 'true';
        }
        if (str_contains((string) $controls, 'mapType')) {
            $mapTypeControl = 'true';
        }
        if (str_contains((string) $controls, 'streetView')) {
            $streetViewControl = 'true';
        }

        if ($api_key = trim((string) $xoTheme->template->_tpl_vars['google-maps-api-key'])) {
            $api_key = '?key=' . $api_key;
        }
        ...
        if ($latlng) {
            // remove white spaces
            $latlng = str_replace(' ', '', (string) $latlng);
    ...
    function sc_tabs($attr, $content = null)
    {
        ...
        if (is_array($tabs_array)) {
            ...
            foreach ($tabs_array as $tab) {
                ...
                $pattern = '/\[tab(\]|s| )/';
                if (preg_match($pattern, (string) $tab['content'])) {
...
if (!function_exists('sc_bar')) {
    function sc_bar($attr, $content = null)
    {
        ...
        if ($size) {
            $size = 'style="height:' . intval($size) . 'px"';
        }
...
if (!function_exists('sc_video')) {
    function sc_video($attr, $content = null)
    {
        ...
        if ($html5_parameters) {
            $html5_parameters = explode(';', (string) $html5_parameters);
