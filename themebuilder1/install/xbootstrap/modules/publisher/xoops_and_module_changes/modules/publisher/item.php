...
$itemObj = $publisher->getHandler('item')
                     ->get($itemid);
...
$categoryObj = $publisher->getHandler('category')
...
if (!$xoopsUser
    || ($xoopsUser->isAdmin(
        ...
        && $publisher->getConfig('item_admin_hits') == 1)
    || ($xoopsUser
        && !$xoopsUser->isAdmin(
            $publisher->getModule()
                      ->mid()
...
if ($publisher->getConfig('item_other_items_type') == "previous_next") {
    ...
    $previousObj = $publisher->getHandler('item')
                             ->getPreviousPublished($itemObj);
    $nextObj     = $publisher->getHandler('item')
...
if ($publisher->getConfig('item_other_items_type') == "all") {
    $itemsObj = $publisher->getHandler('item')
...
foreach ($filesObj as $fileObj) {
    ...
    if ($fileObj->mimetype() == 'application/x-shockwave-flash') {
        ...
        if (strpos((string) $item['maintext'], '[flash-' . $fileObj->getVar('fileid') . ']')) {
            $item['maintext'] = str_replace('[flash-' . $fileObj->getVar('fileid') . ']', $file['content'], (string) $item['maintext']);
        } else {
            $embeded_files[] = $file;
        }
    } else {
...
$xoopsTpl->assign(
    'sectionname', $publisher->getModule()
                             ->getVar('name')
);
$xoopsTpl->assign(
    'modulename', $publisher->getModule()
                            ->getVar('dirname')
);
...
if ((($itemObj->cancomment() == 1) || !$publisher->getConfig('perm_com_art_level')) && ($publisher->getConfig('com_rule') <> 0)) {
    ...
    $xoopsTpl->_tpl_vars['commentsnav'] = str_replace("self.location.href='", "self.location.href='" . PUBLISHER_URL . '/', (string) $xoopsTpl->_tpl_vars['commentsnav']);
}
