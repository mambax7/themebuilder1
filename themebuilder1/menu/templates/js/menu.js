jQuery(function($) {
	...
	//added to modify css affichage a modifier focus info url
	$('.ns-url').live('click', function() {
		var url = $(this).closest('.ns-url').text();
						...
						success: function(data) {
							switch (data.status) {
								case 1:
									...
									menu_div.find('.ns-icon').html('<span class="mfn-icon" data-rel="' +data.menu.icon+ '"><i class="' +data.menu.icon+ '"></i></span>');
									break;
								case 2:
									gbox.hide();
									break;
						...
						$.ajax({
							...
							success: function(data) {
								...
								switch (data.status) {
									case 1:
										gbox.hide();
										$('#menu-group').append('<li><a href="admin.php?fct=themebuilder1&op=menu&action=menumanager&group_id=' + data.id + '">' + group_title + '</a></li>');
										break;
									case 2:
			...
			buttons: {
				'Yes': function() {
					$.post(site_url('menu_group.delete'), param, function(data) {
						if (data.success) {
							window.location = 'admin.php?fct=themebuilder1&op=menu&action=menumanager';
						} else {
							gbox.show({
								content: 'Failed to delete this menu.'
							});
