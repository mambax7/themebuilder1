...
if ($method == 'load_thumbs') {
    ...
    foreach ($files as $k => $file) {
        if (in_array($file, $exclude)) {
            ...
            $data[] = ['id' => $k, 'title' => strtolower((string) $file), 'date' => $date, 'size' => $size, 'type' => $file_ext, 'url' => strtolower($file_url), 'thumb' => strtolower((string) $thumb)];
            $i++;
        }
    }
...
if ($method == 'mlib_create_import_method') {
    $title = htmlentities((string) $_POST['name'], ENT_QUOTES, "UTF-8");
    $data  = htmlentities((string) $_POST['data'], ENT_QUOTES, "UTF-8");
...
if ($method == 'mlib_single_edit') {
    ...
    $caption       = htmlentities((string) $_POST['caption'], ENT_QUOTES, "UTF-8");
...
if ($method == 'mlib_save_type') {
    $title   = htmlentities((string) $_POST['title'], ENT_QUOTES, "UTF-8");
    $content = htmlentities((string) $_POST['content'], ENT_QUOTES, "UTF-8");
