...
if (!class_exists('MFN_Options')) {
    if (!defined('MFN_OPTIONS_DIR')) {
        define('MFN_OPTIONS_DIR', __DIR__);
    }
    ...
    class MFN_Options
    {
         ...
         */
        function __construct(public $menu = [], public $sections = [])
        {
            $defaults = [];
...
//            $fetch_options = $fetch['conf_value'];
            $fetch_options = null;
            if (is_array($fetch) && isset($fetch1['conf_value'])) {
                // Access the array offset here
                $fetch_options = $fetch['conf_value'];
            } else {
                // Handle the case where $fetch1 is not an array or the index does not exist
            }
...
';
        ...
        function _download_options()
        {
            ...
            if (isset($_GET['action']) && $_GET['action'] == 'download_options') {
                ...
                header('Content-Disposition: attachment; filename="' . str_replace('mfn-opts-', '', (string) $_GET['feed']) . '_options_' . date('d-m-Y') . '.txt"');
